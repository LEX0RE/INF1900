...
USART::USART(unsigned int baud, bool modeStopBit, unsigned int modeBitEnvoye){
  *UBRRnH_ = (baud >> 8);
  *UBRRnL_ = baud;
...
USART0::USART0(unsigned int baud) : USART(baud){
  modifierRegistre();
}
...
USART1::USART1(unsigned int baud) : USART(baud){
  modifierRegistre();
}
